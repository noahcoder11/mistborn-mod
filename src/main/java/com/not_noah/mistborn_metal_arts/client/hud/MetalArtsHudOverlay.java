...
public final class MetalArtsHudOverlay {
    ...
    public static void render(ForgeGui gui, GuiGraphics graphics, float partialTick, int screenWidth, int screenHeight) {
        ...
        boolean hasAllomancy = !data.allomanticPowers().isEmpty();
        boolean hasFeruchemy = !data.feruchemicalPowers().isEmpty();

        if (!hasAllomancy && !hasFeruchemy) {
            return;
        }
        ...
        // Allomancy stack (Top Left Corner)
        int allomancyX = 20;
        int allomancyY = 20;
        int maxAllomancyY = 20;
        ...
        if (hasAllomancy) {
            for (Metal metal : Metal.cachedValues()) {
                ...
                if (reserve > 0F || burning || (isSelected && data.hasAllomanticPower(metal))) {
                    if (allomancyX + 12 > screenWidth / 2) {
                        allomancyX = 20;
                        allomancyY += 32; // Wrap to next row
                    }
                    float capacity = ServerConfig.reserveCapacity(metal);
                    drawCircularGauge(graphics, font, allomancyX, allomancyY, metal, reserve, capacity, burning, flaring, isSelected, true, data);
                    allomancyX += 24;
                    if (allomancyY > maxAllomancyY) {
                        maxAllomancyY = allomancyY;
                    }
                }
            }
        }

        // 2. Draw Feruchemy Gauges (Top Right Corner)
        if (hasFeruchemy) {
            int feruchemyX = screenWidth - 20;
            int feruchemyY = 20;
            for (Metal metal : Metal.cachedValues()) {
                ...
                if (totalCharge > 0F || active || (isSelected && data.hasFeruchemicalPower(metal))) {
                    if (feruchemyX - 12 < screenWidth / 2) {
                        feruchemyX = screenWidth - 20;
                        feruchemyY += 32; // Wrap to next row
                    }
                    drawCircularGauge(graphics, font, feruchemyX, feruchemyY, metal, totalCharge, totalCapacity, active, false, isSelected, false, data);
                    feruchemyX -= 24;
                }
            }
        }
        ...
        // 4. Render Spiritweb Status Panel (Significantly Minimized: Hidden unless in critical warning states)
        float stability = data.soulStability();
        float contamination = data.identityContamination();
        float bloat = data.spiritualBloat();

        if (stability < 50.0F || contamination > 50.0F || bloat > 50.0F) {
            int startX = 10;
            int startY = maxAllomancyY + 20; // Dynamically shifted below Allomancy rows
            String warningStr = "";
            int warningColor = 0xFFFFFFFF;
            if (stability < 30.0F) {
                warningStr = "SPIRIT Collapse Alert (Stability: " + Math.round(stability) + "%)";
                warningColor = 0xFFFF3D00;
            } else if (contamination > 75.0F) {
                warningStr = "Identity Contaminated (" + Math.round(contamination) + "%)";
                warningColor = 0xFFB388FF;
            } else if (bloat > 75.0F) {
                warningStr = "Spiritual Bloated (" + Math.round(bloat) + "%)";
                warningColor = 0xFFFF80AB;
            } else {
                warningStr = "Spiritual Instability Detected";
                warningColor = 0xFFFFD54F;
            }
            graphics.drawString(font, warningStr, startX, startY, warningColor, true);
        }
    }
    ...
    private static void drawCircularGauge(GuiGraphics graphics, Font font, float cx, float cy, Metal metal, float reserve, float capacity, boolean active, boolean flaring, boolean isSelected, boolean isAllomancy, MetalArtsData data) {
        float r_out = 9.5F;
        float r_in = 7.0F;
        ...
        // Draw progress arc
        float fillPct = capacity > 0F ? Math.min(1.0F, reserve / capacity) : 0F;
        ...
        if (!isAllomancy) {
            if (data.feruchemyMode(metal) != 0) {
                int modeVal = data.feruchemyMode(metal);
                String txt = modeVal < 0 ? "S" + (-modeVal) : "x" + modeVal;
