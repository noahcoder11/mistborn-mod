...
public class SpikeRemovalToolItem extends Item {
    public SpikeRemovalToolItem(Properties properties) {
        super(properties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (!level.isClientSide && player instanceof ServerPlayer serverPlayer) {
            serverPlayer.getCapability(MetalArtsCapabilities.METAL_ARTS).ifPresent(data -> {
                if (HemalurgyManager.removeLastSpike(serverPlayer, data) && !serverPlayer.getAbilities().instabuild) {
